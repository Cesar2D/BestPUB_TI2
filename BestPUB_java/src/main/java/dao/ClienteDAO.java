...
public class ClienteDAO {
    ...
    public static boolean insertCliente(Cliente cliente) {
        ...
        try {
            ...
            st.executeUpdate("INSERT INTO cliente (cpf,nome,ddn,senha,email) " +
                    "VALUES (" + cliente.getCpf() + ", '" + cliente.getNome() + "','" + cliente.getDdn() + "','"
                    + cliente.getSenha() + "','" + cliente.getEmail() + "');");
            st.close();
            status = true;
    ...
    public static boolean updateCliente(Cliente cliente) {
        ...
        try {
            Statement st = conexao.createStatement();
            String sql = "UPDATE cliente SET nome = '" + cliente.getNome() + "', ddn = '" + cliente.getDdn()
                    + "', senha = '" + cliente.getSenha() + "','" + cliente.getEmail() + "'" + "WHERE cpf = "
                    + cliente.getCpf();
    ...
    public static Cliente getCliente(String cpf) {
        ...
        try {
            ...
            if (rs.next()) {
                rs.last();
                cliente = new Cliente(rs.getString("cpf"), rs.getString("nome"), rs.getString("ddn"),
                        rs.getInt("idade"), rs.getString("senha"), rs.getString("email"));
                rs.beforeFirst();
            }
        } catch (Exception e) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                for (int i = 0; rs.next(); i++) {
                    clientes[i] = new Cliente(rs.getString("cpf"), rs.getString("nome"), rs.getString("ddn"),
                            rs.getInt("idade"), rs.getString("senha"), rs.getString("email"));
                }
            }
            st.close();
